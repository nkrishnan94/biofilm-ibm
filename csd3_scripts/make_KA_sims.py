...
z_length = np.array([3,4,5,6,7,8,9])
x_length = np.array([140])
deform_scale = np.array([.6])

temps = np.array([0.1,0.15,0.2])
        ...
        for z in z_length:
            for n in range(samples):
                for t in temps:
                    ...
                    f.write('create_atoms 1 random ' +str(int(6*x*.75*z)) +' '+str(n+1)+' '+'film\n') # Create atoms at random within film.
                    f.write('create_atoms 2 random ' +str(int(6*x*.75*z)) +' '+str(10000+n+1)+' '+'film\n') # Create atoms at random within film.
