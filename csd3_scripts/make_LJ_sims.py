...
z_length = np.array([6])
x_length = np.array([50,75,100,125,150,175,200,225,250])
		...
		for z in z_length:
		    for n in range(samples):
		    	for t in temps:
			        ...
			        file_str=str(d_1)+"_thick_"+str(z)+"_len_"+str(x)+"_scale_"+str(s)+'_rep_'+str(n)+'_temp_'+str(t)
			        f = open(new_path+"/biofilm_LJ_"+str(cnt)+".inp", "a")
			        f.write('#-------------------------- Initial Setup -------------------------------------#\n')
			        f.write('log ' +new_path+'/log_LJ_'+str(cnt)+'_'+file_str+'.log\n')
			        f.write('units       lj\n')
			        ...
			        f.write('region film block 0 '+ str(x) +' 0 '+str(z)+' 0 6 \n')# Geometrical region.
			        f.write('create_box 1 film \n')# Create simulation box.
			        f.write('create_atoms 1 random ' +str(int(6*x*1.5*z)) +' '+str(n+1)+' '+'film\n') # Create atoms at random within film.
                    ...
                    f.write('fix 10 all nvt temp 3 ' +str(t)+ ' $(100.0*dt)\n')
                    f.write('run 5000\n')
                    f.write('unfix 10\n')

                    f.write('fix 20 all npt temp '+str(t)+' '+str(t) +' $(100.0*dt) y 0 0 $(1000.0*dt)\n')
                    f.write('run 5000\n')
                    f.write('unfix 20\n')

                    f.write('change_box all y final -40 40\n')

                    f.write('fix 30 all nvt temp '+str(t)+' '+str(t) +' $(100.0*dt)\n')
                    f.write('run 10000\n')

                    f.write('fix 40 all deform 1 x scale '+str(s)+' remap x units box \n')
                    f.write('run '+str(30000)+' \n')
